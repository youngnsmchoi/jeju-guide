'use client'
// 삼각김밥 항목(gimbap_items) 관리 — 목록, 추가/수정/삭제
...
type FormState = Omit<GimbapItem, 'id'> & { id?: number }
...
export default function GimbapAdmin() {
  const [items, setItems] = useState<GimbapItem[]>([])
  ...
  const load = async () => {
    const data = await fetch('/api/admin/gimbap').then(r => r.json())
  ...
  const startEdit = (item: GimbapItem) => setForm({ ...item })
  ...
  const save = async () => {
    ...
    const res = await fetch('/api/admin/gimbap', {
      method,
  ...
  const deleteItem = async (id: number) => {
    ...
    await fetch(`/api/admin/gimbap?id=${id}`, { method: 'DELETE' })
    await load()
  }
      ...
      {!form && (
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold text-gray-700">삼각김밥 목록</p>
            <button
              onClick={startNew}
              ...
              {items.map(item => (
                <div key={item.id} className="flex items-center justify-between border border-gray-100 rounded-xl px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-gray-800">{getGimbapField(item, 'name', 'ko')}</span>
