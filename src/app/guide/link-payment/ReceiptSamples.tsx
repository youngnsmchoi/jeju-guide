...
const LABEL: Record<Lang, {
  ...
  ko: {
    ...
    body: {
      basic: [
        '1+1 대상이 아닌 상품(파워에이드)은 2개를 사면 그냥 정가로 2개 값이 찍힙니다.',
        '1+1 대상 상품(게토레이)은 1개만 계산대에 올리면, 점원이 한국어로 "1개 더 가져오셔야 해요"라고 안내합니다. 말이 안 통하면 서로 곤란해질 수 있어요.',
      ...
      mix: [
        '1+1 대상 음료(게토레이)와 전혀 다른 상품(아이스크림, 담배용품 등)을 함께 계산해도, 1+1은 정확히 대상 상품끼리만 적용됩니다.',
        '순서 상관없이 섞어서 계산대에 올려도 괜찮습니다.',
        '어떤 상품이 1+1 대상인지는 진열대에 붙어있는 "숏카드"(가격 안내 카드)를 보면 알 수 있습니다.',
      ],
      alcohol: [
        '주류는 1+1보다 "여러 개 사면 할인" 방식이 많습니다.',
        '이 영수증 예시: 맥주 500ml 1개 정가 4,000원, 4개 사면 16,000원이 찍힌 뒤 "상품할인 -6,000원"이 적용되어 실제로는 10,000원(1개당 2,500원)만 결제됩니다.',
      ...
      bulk: [
        ...
        '이 영수증 예시: 1,500원짜리 아이스크림 5개(정가 7,500원)를 구입 → 5개 구매 시 50% 할인이 적용되어 3,750원만 결제됩니다.',
  ...
  en: {
    ...
    body: {
      basic: [
        'For items NOT part of a 1+1 deal (Powerade), buying 2 just charges full price for both.',
        'For 1+1 items (Gatorade), if you only bring 1 to the counter, the clerk will tell you in Korean to bring one more. This can be confusing if you don\'t speak Korean.',
      ...
      mix: [
        'Even if you check out a 1+1 drink (Gatorade) together with completely different items (ice cream, tobacco accessories, etc.), the 1+1 discount still applies correctly, only to the matching items.',
      ...
      alcohol: [
        ...
        'Example from this receipt: a 500ml beer is 4,000 won each. Buying 4 shows 16,000 won, then a "product discount of -6,000 won" is applied, so you actually pay 10,000 won (2,500 won each).',
      ...
      bulk: [
        ...
        'Example from this receipt: five 1,500-won ice creams (7,500 won full price) — buying 5 triggers a 50% discount, so you pay only 3,750 won.',
  ...
  zh: {
    ...
    body: {
      basic: [
        '非1+1商品（Powerade）买2个的话，就按原价计算2个的价格。',
        '1+1商品（佳得乐）如果只拿1个去结账，店员会用韩语告诉你需要再拿一个。如果语言不通，可能会造成沟通困难。',
      ...
      mix: [
        '即使把1+1饮料（佳得乐）和完全不同的商品（冰淇淋、烟草用品等）一起结账，1+1优惠依然只会准确应用于符合条件的商品上。',
        '可以不按顺序混合放在收银台，没有问题。',
        '货架上的"价签卡"（价格说明卡）会标明哪些商品适用1+1。',
      ],
      alcohol: [
        '酒类比起1+1，更常见"买多个打折"的方式。',
        '本收据示例：500ml啤酒单价4,000韩元，买4个显示16,000韩元，随后应用"商品折扣-6,000韩元"，实际只需支付10,000韩元（每个2,500韩元）。',
      ...
      bulk: [
        ...
        '本收据示例：购买5个1,500韩元的冰淇淋（原价7,500韩元）→ 购买5个可享受50%折扣，实际只需支付3,750韩元。',
  ...
  ja: {
    ...
    body: {
      basic: [
        '1+1対象外の商品（パワーエイド）を2個買うと、そのまま定価で2個分の金額になります。',
        '1+1対象商品（ゲータレード）を1個だけレジに持っていくと、店員が韓国語で「もう1個持ってきてください」と案内します。言葉が通じないと困ることがあります。',
      ...
      mix: [
        '1+1対象の飲料（ゲータレード）と全く違う商品（アイスクリーム、タバコ用品など）を一緒に会計しても、1+1は対象商品同士にのみ正確に適用されます。',
        '順番を気にせず混ぜてレジに出しても問題ありません。',
        'どの商品が1+1対象かは、棚に貼られている「ショートカード」（価格案内カード）で確認できます。',
      ],
      alcohol: [
        '酒類は1+1より「複数購入で割引」の方式が多いです。',
        'このレシート例：ビール500ml1個の定価は4,000ウォン、4個買うと16,000ウォンと表示された後「商品割引-6,000ウォン」が適用され、実際には10,000ウォン（1個あたり2,500ウォン）のみ支払います。',
      ...
      bulk: [
        ...
        'このレシート例：1,500ウォンのアイスクリームを5個購入（定価7,500ウォン）→ 5個購入で50%割引が適用され、実際には3,750ウォンのみ支払います。',
