...
const LABEL: Record<Lang, {
  ...
  error: string; alreadyLogged: string; loading: string; noData: string;
  total: string;
}> = {
  ko: {
    title: 'My Ramen Log',
  ...
  en: {
    ...
    q3: 'How was it?',
    submit: 'Done',
    privacy: 'Your response is collected as anonymous statistics by country.',
    done: 'Thank you!',
  ...
  ja: {
    ...
    q3: '味はいかがでしたか？',
    submit: '完了',
    privacy: 'ご回答は国別の匿名統計としてのみ使用されます。',
    done: 'ありがとうございます！',
...
function StatsTab({ lang }: { lang: Lang }) {
  ...
  if (stats.length === 0) return <p className="text-center text-gray-400 py-12 text-sm">{L.noData}</p>
        ...
        {tab === 'log' && (
          <>
            ...
            <div>
              ...
              <select
                value={ramenId}
                onChange={e => { setRamenId(e.target.value); setAlreadyLogged(false) }}
