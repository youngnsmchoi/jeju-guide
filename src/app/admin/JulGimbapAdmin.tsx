...
type FormState = Omit<JulGimbapItem, 'id'> & { id?: number }
...
export default function JulGimbapAdmin() {
  const [items, setItems] = useState<JulGimbapItem[]>([])
  ...
  const load = async () => {
    const data = await fetch('/api/admin/jul-gimbap').then(r => r.json())
  ...
  const startEdit = (item: JulGimbapItem) => setForm({ ...item })
  ...
  const save = async () => {
    ...
    const res = await fetch('/api/admin/jul-gimbap', {
      method,
  ...
  const deleteItem = async (id: number) => {
    ...
    await fetch(`/api/admin/jul-gimbap?id=${id}`, { method: 'DELETE' })
    await load()
  }
              ...
              {items.map(item => (
                <div key={item.id} className="flex items-center justify-between border border-gray-100 rounded-xl px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-gray-800">{getJulGimbapField(item, 'name', 'ko')}</span>
