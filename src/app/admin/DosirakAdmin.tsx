'use client'
// 도시락 항목(dosirak_items) 관리 — 목록, 추가/수정/삭제
...
const TEXT_FIELDS = [
  ...
  { key: 'flavor_desc' as const, label: '맛 설명' },
  { key: 'composition' as const, label: '구성' },
  { key: 'allergen_note' as const, label: '알레르기/성분 안내' },
]

type FormState = Omit<DosirakItem, 'id'> & { id?: number }
...
const emptyForm = (orderNum: number): FormState => ({
  ...
  composition_ko: '', composition_en: '', composition_zh: '', composition_ja: '',
  allergen_note_ko: '', allergen_note_en: '', allergen_note_zh: '', allergen_note_ja: '',
  price_krw: null,
  manufacturer_url: '',
})

export default function DosirakAdmin() {
  const [items, setItems] = useState<DosirakItem[]>([])
  ...
  const load = async () => {
    const data = await fetch('/api/admin/dosirak').then(r => r.json())
  ...
  const startEdit = (item: DosirakItem) => setForm({ ...item })
  ...
  const save = async () => {
    ...
    const res = await fetch('/api/admin/dosirak', {
      method,
  ...
  const deleteItem = async (id: number) => {
    ...
    await fetch(`/api/admin/dosirak?id=${id}`, { method: 'DELETE' })
    await load()
  }
      ...
      {!form && (
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold text-gray-700">도시락 목록</p>
            <button
              onClick={startNew}
          ...
          {items.length === 0 ? (
            <p className="text-center text-gray-400 py-8 text-sm">등록된 항목이 없습니다.</p>
          ) : (
            <div className="space-y-2">
              {items.map(item => (
                <div key={item.id} className="flex items-center justify-between border border-gray-100 rounded-xl px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-gray-800">{getDosirakField(item, 'name', 'ko')}</span>
                    {(!item.price_krw || item.price_krw === 0) && (
                      <span className="text-xs bg-amber-100 text-amber-700 rounded px-1.5 py-0.5">가격 미입력</span>
                    )}
                  </div>
      ...
      {form && (
        <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs text-gray-400 font-medium">가격 (원) — 실제 조사 후 입력</label>
              <input
                type="number"
