'use client'
// 핫바 항목(hotbar_items) 관리 — 목록, 추가/수정/삭제
...
type FormState = Omit<HotbarItem, 'id'> & { id?: number }
...
export default function HotbarAdmin() {
  ...
  const load = async () => {
    const data = await fetch('/api/admin/hotbar').then(r => r.json())
  ...
  const startEdit = (item: HotbarItem) => setForm({ ...item })
  ...
  const save = async () => {
    ...
    const res = await fetch('/api/admin/hotbar', {
      method,
  ...
  const deleteItem = async (id: number) => {
    ...
    await fetch(`/api/admin/hotbar?id=${id}`, { method: 'DELETE' })
    await load()
  }
      ...
      {!form && (
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold text-gray-700">핫바 목록</p>
            <button
              onClick={startNew}
              ...
              {items.map(item => (
                <div key={item.id} className="flex items-center justify-between border border-gray-100 rounded-xl px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-gray-800">{getHotbarField(item, 'name', 'ko')}</span>
