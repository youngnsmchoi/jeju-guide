              ...
              <div className="grid grid-cols-2 gap-3">
                {results.map(item => (
                  <div key={item.id} className="bg-white rounded-2xl border border-gray-200 p-3">
                    <p className="text-xs font-bold text-gray-900 leading-snug">{getRamenField(item, 'name', lang)}</p>
                    {item.price_krw && <p className="text-xs text-gray-400 mt-1">₩{item.price_krw.toLocaleString()}</p>}
                  </div>
                ))}
              </div>
            )}
